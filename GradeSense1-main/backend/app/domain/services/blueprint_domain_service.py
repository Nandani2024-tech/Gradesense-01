"""Domain service for blueprint-related business rules."""
...
def derive_blueprint_health(exam_doc: Dict[str, Any], questions: List[dict]) -> dict:
    """Domain-level helper to derive blueprint health using inferred counts."""
...
def format_blueprint_lock_failure(exam: dict, readiness: dict, context: str) -> dict:
    """Unified error formatter for blueprint lock failures."""
    health = readiness.get("health") or {}
    issues = readiness.get("issues") or []
    return {
        "message": (
            f"Cannot {context}: blueprint is not locked. "
