...
from app.services.pipelines.ai_structured.college.alignment import align_packets_to_blueprint
from app.services.pipelines.ai_structured.college.blueprint import assemble_blueprint
from app.services.pipelines.ai_structured.college.contracts import CollegePipelineResult
...
from app.services.pipelines.ai_structured.college.college_recovery import build_gate, compute_confidence_vectors, run_recovery
from app.services.pipelines.ai_structured.universal.region_ocr import extract_region_text
from app.services.pipelines.ai_structured.college.packet_builder import build_packets
from app.services.pipelines.ai_structured.college.structuring import structure_aligned_answers
...
class CollegePipelineV2:
    ...
    async def run(
        ...
        # ADDED LOGGING START
        logger.info("[PIPELINE START] COLLEGE_PIPELINE | exam_id=%s | submission_id=N/A", exam_id)
        # ADDED LOGGING END
        logger.info("[COLLEGE-V2] run start exam_id=%s pages=%s", exam_id, len(answer_images or []))

        # Phase 1: blueprint
        # ADDED LOGGING START
        logger.info("[STEP START] ASSEMBLE_BLUEPRINT")
        # ADDED LOGGING END
        ...
        # ADDED LOGGING START
        logger.info("[STEP SUCCESS] ASSEMBLE_BLUEPRINT")
        # ADDED LOGGING END
        ...
        # ADDED LOGGING START
        logger.info("[STEP START] NORMALIZE_PAGES")
        # ADDED LOGGING END
        phase_start = time.perf_counter()
        ...
        # ADDED LOGGING START
        logger.info("[STEP SUCCESS] NORMALIZE_PAGES")
        # ADDED LOGGING END
        if not clean_pages:
            gate = {
                "mapping_status": "failed",
        ...
        # ADDED LOGGING START
        logger.info("[PIPELINE END] COLLEGE_PIPELINE")
        # ADDED LOGGING END
