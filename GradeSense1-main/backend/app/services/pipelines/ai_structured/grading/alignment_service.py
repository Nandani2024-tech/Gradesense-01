...
def _extract_objective_from_ocr(
    ...
    for page_idx, img in enumerate(answer_images):
        try:
            ...
            res = await ocr_service.extract_text(img)
...
async def align_answers(
    *,
    ...
    # ADDED LOGGING START
    logger.info("[STEP START] OBJECTIVE_FALLBACK")
    # ADDED LOGGING END
    objective_fallback = _extract_objective_from_ocr(question_structure, answer_images, ocr_service)
    # ADDED LOGGING START
    logger.info("[STEP SUCCESS] OBJECTIVE_FALLBACK")
    # ADDED LOGGING END
