...
async def grade_pdf(
    ...
    try:
        ...
        # ----------------------------
        logger.info("grade_pdf: starting extraction for pdf")
        ...
        logger.info(f"grade_pdf: extracted {len(packets)} packets")
        ...
        if result.get("total_possible", DEFAULT_TOTAL_POSSIBLE) == DEFAULT_TOTAL_POSSIBLE and result.get("grades"):
            logger.warning("grade_pdf: total_possible was zero, recomputing from grades")
        ...
        logger.info(f"grade_pdf: result awarded={result.get('total_awarded')} possible={result.get('total_possible')}")
        return result
