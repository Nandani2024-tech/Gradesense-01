...
async def _process_question_paper_async(exam_id: str):
    ...
    try:
        ...
        blueprint_status = result.get("blueprint_status", "ready_unlocked" if result.get("success") else "failed")
        update_data = {
        ...
        # Process model answer if exists
        ...
        if MARK_VALIDATION_ENABLED:
            try:
                ...
                if validator_payload:
                    extracted_questions = await db.questions.find({"exam_id": exam_id}).to_list(2000)
                ...
                logger.warning(f"Mark validation failed: {e}")
        ...
        if teacher_id:
            await create_notification(
                ...
                message=f"Extracted questions for {exam_name}",
                link=f"/teacher/review?exam={exam_id}"
            )

    except Exception as e:
        logger.error(f"[QP-ASYNC] Failed for exam {exam_id}: {e}", exc_info=True)
        await db.exams.update_one({"exam_id": exam_id}, {"$set": {"question_extraction_status": "failed", "question_paper_processing": False}})
