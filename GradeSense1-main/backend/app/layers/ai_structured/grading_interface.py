...
async def _llm_json(prompt: str, model_name: str, images: Optional[List[str]] = None) -> Dict[str, Any]:
    api_key = get_llm_api_key()
    if not api_key:
        raise RuntimeError("missing_gemini_api_key")

    provider = os.getenv("GRADING_LLM_PROVIDER") or os.getenv("LLM_PROVIDER", "gemini").lower()
    actual_model = model_name
    
    if os.getenv("GRADING_LLM_PROVIDER") == "gemini":
        # User explicitly requested Gemini for grading
        actual_model = os.getenv("GRADING_MODEL_NAME", "gemini-2.5-flash")
        provider = "gemini"
    elif provider == "ollama":
        # Default fallback for Ollama if no specific grading provider is set
        actual_model = "qwen2.5:latest"
    elif provider == "gemini":
        # Normal Gemini flow
        actual_model = os.getenv("GEMINI_MODEL_NAME", "gemini-2.5-flash")

    chat = LlmChat(
        api_key=api_key or "no-key",
        session_id=f"ai_struct_grade_{uuid.uuid4().hex[:10]}",
        system_message="Return strict JSON only.",
    ).with_model(provider, actual_model).with_params(temperature=0)

    file_contents = [ImageContent(image_base64=img) for img in (images or [])]
    if provider == "ollama" and "vision" not in actual_model.lower():
        # Strip images if the model is not vision-capable to avoid Ollama errors.
        file_contents = []
...
async def infer_objective_key_consensus(
    *,
    ...
    for _ in range(max(1, runs)):
        ...
        try:
            payload = await _llm_json(prompt, model_name, images=context_images)
